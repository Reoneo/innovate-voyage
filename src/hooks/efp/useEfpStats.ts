...
import { fetchEfpStats, fetchEfpFollowers, fetchEfpFollowing, processUsers } from "./efpApi";
import { shortenAddress } from "./efpUtils";
import type { EfpStats, EfpPerson } from "./efpTypes";
...
export function useEfpStats(walletAddress?: string) {
  ...
  const { followLoading, setFollowLoading, followAddress: followAddressAction } = useFollowAction();
  ...
  const followAddress = async (addressToFollow: string): Promise<void> => {
    ...
    setFollowLoading(prev => ({ ...prev, [addressToFollow]: true }));
    
    try {
      await followAddressAction(addressToFollow);
      ...
      setFollowLoading(prev => ({ ...prev, [addressToFollow]: false }));
    }
  };
