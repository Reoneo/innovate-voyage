...
const PoapSection: React.FC<PoapSectionProps> = ({
  ...
  return <section className="w-full flex flex-col items-center">
      {/* POAP count display */}
      {poaps.length > 0 && !isLoading && <div className="text-sm text-center mb-2 text-muted-foreground">
          <span className="font-medium text-primary">{poaps.length}</span> POAPs collected
        </div>}

      <div className="relative w-full aspect-square flex items-center justify-center rounded-sm py-0 bg-inherit">
