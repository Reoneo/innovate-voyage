...
const MobileActivityColumn: React.FC<MobileActivityColumnProps> = ({
  ...
  } = useWebacyData(passport?.owner_address);
  const {
    data: transactions
  } = useLatestTransactions(passport?.owner_address, 10);
      ...
      <Dialog open={showActivityModal} onOpenChange={setShowActivityModal}>
        <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto bg-white border-0 shadow-2xl">
          <DialogHeader className="relative border-b border-gray-100 pb-6">
            ...
            <Button variant="ghost" size="icon" className="absolute right-0 top-0 h-10 w-10 rounded-full hover:bg-gray-100" onClick={() => setShowActivityModal(false)}>
              <X className="h-5 w-5" />
            </Button>
          </DialogHeader>
          ...
          <div className="mt-8 space-y-8">
            ...
            <div className="bg-gradient-to-br from-red-50 via-orange-50 to-yellow-50 border border-red-100 rounded-2xl p-6 shadow-sm">
              ...
              {webacyLoading ? <div className="animate-pulse space-y-3">
                  <div className="h-6 bg-gray-200 rounded-lg w-3/4"></div>
                  <div className="h-4 bg-gray-200 rounded-lg w-1/2"></div>
                </div> : securityData ? <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Badge variant={securityData.threatLevel === 'LOW' ? 'secondary' : 'destructive'} className="text-lg py-2 px-4 font-semibold">
            ...
            <div className="bg-gradient-to-br from-blue-50 via-cyan-50 to-teal-50 border border-blue-100 rounded-2xl p-6 shadow-sm">
              ...
              {transactions && transactions.length > 0 ? <div className="space-y-4 max-h-80 overflow-y-auto">
                  {transactions.slice(0, 8).map(tx => <div key={tx.hash} className="bg-white rounded-xl p-4 border border-blue-100 shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                          <div className={`w-3 h-3 rounded-full ${tx.from.toLowerCase() === passport.owner_address.toLowerCase() ? 'bg-red-500' : 'bg-green-500'}`}></div>
