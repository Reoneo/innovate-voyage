...
const ProfileAvatar: React.FC<ProfileAvatarProps> = ({
  ...
  return <Avatar className={`h-48 w-48 border-2 border-white shadow-md mx-auto relative profile-avatar ${isBoxUser ? '' : 'rounded-full'}`}>
      ...
      {!imageError && finalAvatarUrl ? <AvatarImage src={finalAvatarUrl} alt={name} onError={handleImageError} onLoad={handleImageLoad} className="" /> : <AvatarImage src="/placeholder.svg" alt={name} className={`object-cover ${isBoxUser ? 'rounded-lg' : ''}`} onError={handlePlaceholderError} onLoad={() => setIsLoading(false)} />}
