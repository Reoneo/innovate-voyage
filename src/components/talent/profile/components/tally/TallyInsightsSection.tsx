  ...
  const renderSummaryContent = () => {
    ...
    return <div className="space-y-3">
        <div className="flex items-center">
          <img src={tallyData.governorInfo.iconUrl || "https://assets.tally.xyz/tally-logo.svg"} alt={tallyData.governorInfo.name} className="h-8 w-8 mr-3 rounded-md" />
          <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="p-3 bg-purple-50 rounded-lg">
            ...
            <p className="text-lg font-bold">{tallyData.votingInfo.receivedDelegations ? "Yes" : "None"}</p>
          </div>
        </div>
        
        {tallyData.votingInfo.recentVotes && tallyData.votingInfo.recentVotes.length > 0 && <div>
            <p className="text-xs text-muted-foreground mb-1">Latest vote:</p>
            <p className="text-sm font-medium truncate">{tallyData.votingInfo.recentVotes[0].proposalTitle}</p>
          </div>}
      </div>;
  };
  return <>
