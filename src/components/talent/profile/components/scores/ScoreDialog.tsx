...
const ScoreDialog: React.FC<ScoreDialogProps> = ({ open, onOpenChange, type, walletAddress, data }) => {
  const { score, webacyData, txCount } = data || {};
  ...
  const isLightTheme = type === 'tally' || type === 'webacy';

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={`${getDialogClass()} max-h-[80vh] overflow-y-auto p-0 ${isLightTheme ? 'bg-white text-black' : 'bg-black text-white'}`}>
        {type !== 'webacy' && (
          <div className={`sticky top-0 z-10 ${isLightTheme ? 'bg-white text-black' : 'bg-black text-white'} flex justify-between items-center pb-2 p-4`}>
            ...
            <Button
              ...
              className={`rounded-full h-8 w-8 ${isLightTheme ? 'text-black hover:bg-gray-200' : 'text-white hover:bg-gray-800'}`}
            >
              <X size={18} />
            </Button>
          </div>
        )}
        <div className={`p-0 ${isLightTheme ? 'bg-white text-black' : 'bg-black'}`}>
          {renderDialogContent()}
        </div>
      </DialogContent>
    </Dialog>
