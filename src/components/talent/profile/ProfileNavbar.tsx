  ...
  return <div className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-100 shadow-sm">
      <div className="max-w-6xl mx-auto px-4 py-2 flex items-center justify-between h-14">
        <div className="flex-none w-14 flex justify-center">
          <Link to="/" className="flex items-center justify-center text-primary font-medium" style={{
          color: avatarColor
        }}>
        ...
        <form onSubmit={handleSearch} className="flex-1 flex justify-center">
          <div className="relative max-w-md w-full">
            ...
            <Input type="text" placeholder="Search ENS username..." value={search} onChange={e => setSearch(e.target.value)} className="pl-10 pr-4 py-2 w-full bg-gray-50 border-gray-200 rounded-full focus:ring-primary focus:border-primary" style={{
            borderColor: avatarColor,
            '--tw-ring-color': avatarColor
          } as React.CSSProperties} />
            <Button type="submit" variant="ghost" size="sm" className="absolute right-2 top-1/2 transform -translate-y-1/2 hover:text-primary/80 px-3 py-1" style={{
            color: avatarColor
          }}>
              Search
            </Button>
