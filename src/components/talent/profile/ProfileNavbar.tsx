...
const ProfileNavbar: React.FC<ProfileNavbarProps> = ({
  ...
  return (
    <div className={`fixed top-0 left-0 right-0 z-50 backdrop-blur-md border-b border-gray-600/20 shadow-sm bg-gray-800/30 ${isMobile ? 'safe-area-inset-top' : ''}`}>
      <div className={`mx-auto px-2 sm:px-4 py-2 flex items-center justify-between ${isMobile ? 'h-12' : 'h-14'} ${isMobile ? 'max-w-full' : 'max-w-6xl'}`}>
        <form onSubmit={handleSearch} className="flex-1 flex items-center justify-center gap-1 sm:gap-2">
          <Link to="/" className="text-white hover:text-gray-300 transition-colors flex-shrink-0">
            <Home className={`${isMobile ? 'h-6 w-6' : 'h-7 w-7'}`} />
          </Link>
          
          <div className={`relative w-full ${isMobile ? 'max-w-none mx-2' : 'max-w-md'}`}>
            <Search className={`absolute left-3 top-1/2 transform -translate-y-1/2 ${isMobile ? 'h-4 w-4' : 'h-5 w-5'} text-gray-400`} aria-hidden="true" />
            <Input 
              type="text" 
              placeholder={isMobile ? "Search ENS..." : "Search ENS username..."} 
              value={search} 
              onChange={e => setSearch(e.target.value)} 
              className={`${isMobile ? 'pl-10 pr-16 py-1 text-sm h-9' : 'pl-12 pr-4 py-2'} w-full bg-gray-700/30 border-gray-600/30 text-white rounded-full focus:ring-white focus:border-white`} 
            />
            <Button 
              type="submit" 
              variant="ghost" 
              size="sm" 
              className={`absolute right-1 top-1/2 transform -translate-y-1/2 text-white hover:text-gray-300 ${isMobile ? 'px-2 py-0.5 text-xs h-7' : 'px-3 py-1'}`}
            >
              {isMobile ? 'Go' : 'Search'}
            </Button>
          </div>
